...
DECLARE
	t_h_year INTEGER := (SELECT year_value FROM holidays.gregorian_to_hijri(make_date(p_year, 1, 1)));
	t_datestamp DATE;

BEGIN
	t_datestamp := holidays.hijri_to_gregorian(t_h_year - 1, p_h_month, p_h_day);
	IF DATE_PART('year', t_datestamp) = p_year THEN
		RETURN NEXT t_datestamp;
	END IF;
	t_datestamp := holidays.hijri_to_gregorian(t_h_year, p_h_month, p_h_day);
	IF DATE_PART('year', t_datestamp) = p_year THEN
		RETURN NEXT t_datestamp;
	END IF;
	t_datestamp := holidays.hijri_to_gregorian(t_h_year + 1, p_h_month, p_h_day);
