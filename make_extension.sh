...
cat > extension/holidays--1.0.sql << EOF
...
DBSETUP_FILES=(
  "db_setup/holidays_schema.pgsql"
...
for sql_file in ${DBSETUP_FILES[*]} ${UTILS_FILES[*]} ${COUNTRIES_FILES[*]}; do
	(cat "${sql_file}"; echo; echo) >> extension/holidays--1.0.sql
done

# awk '{gsub(/ holidays\./," @extschema@.");}1' extension/holidays--1.0.sql
